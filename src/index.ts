      ...
      if (match) {
        ...
        for (const ext of EXTENSIONS) {
          ...
          if (fs.existsSync(f) && (!callback || callback(f))) {
            fs.unlinkSync(f);
            if (options.verbose) console.log(`Removed "${colors.yellow(f)}"`);
          }
        }
      }
    });
  if (options.verbose) console.log(`Watching in "${colors.yellow(src)}"`);
}
...
export function cleanMatch(
  ...
  if (options.removeEmptyDirs) _removeDir(root, verbose);
}
...
export function cleanSrc(options: ICleanSrcOptions, callback?: CleanCallback) {
  ...
  const removeFile = (f: string) => {
    if (!callback || callback(f)) {
      fs.unlinkSync(f);
      if (verbose) console.log(`Removed file "${f}"`);
    }
  };
...
function _glob(
  ...
  const files = fg.sync(pattern, {
    ...
    onlyFiles: true,
    absolute: true,
    ignore: Array.isArray(exclude) ? exclude : exclude ? [exclude] : undefined,
  });
...
function _removeDir(root: string, verbose?: boolean) {
  const files = fg.sync(`**/*`, { cwd: root, onlyDirectories: true });
  for (const f of files) {
    const relPath = path.join(root, f);
    if (!fs.readdirSync(relPath).length) {
      fs.rmdirSync(relPath);
      if (verbose) console.log(`Removed directory "${f}"`);
    }
  }
}
